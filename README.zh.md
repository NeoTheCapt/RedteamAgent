...
RedTeam Agent 是一个支持 **Claude Code**、**OpenCode** 和 **Codex** 的自主红队模拟 Agent。它可以把任意工作空间转换成完整的渗透测试环境，面向 CTF / 靶场目标，内置 **8 个 AI Agent**、**容器化 Kali 工具链**、**流式用例收集管道**，以及 **78 份安全参考资料**。
...
- **自主工作流** — 5 阶段方法论（侦察 → 收集 → 测试 → 利用+OSINT → 报告），尽量减少人工干预
...
- **用例收集管道** — 基于 SQLite 的队列，4 个生产者，自动类型分类，调度零 token 消耗
- **78 份参考资料** — 涵盖 OWASP Top 10:2025、API Security 2023、攻击战术、AD/Kerberos 等内容
...
该命令会自动克隆仓库，把对应产品文件安装到 `~/redteam-agent`，在需要时构建 Docker 镜像，并执行基础验证。
...
./install.sh opencode ~/my-project     # 指定安装目录
./install.sh --dry-run opencode        # 只验证，不写文件
./install.sh --force docker            # 强制重建相关镜像
./install.sh -h                        # 查看安装帮助
```

Windows 为明确不支持平台。请使用 macOS 或 Linux 完成安装与运行。
...
> **OpenCode 用户**：请在 `.opencode/opencode.json` 中配置你的 LLM provider：
> ```json
> { "model": "anthropic/claude-sonnet-4-6", "small_model": "anthropic/claude-haiku-4-5-20251001" }
> ```
>
> **OpenCode 可选 Metasploit 能力**：OpenCode 安装路径会附带本地 `metasploit` MCP server，底层连接的是容器化 `msfrpcd`。它只用于 `Exploit` 阶段，在 finding 已经明确匹配某个 Metasploit 模块族、服务、产品/版本或 CVE 时才会使用；不会用于盲目大范围喷射。

### 单镜像运行模式

对于 OpenCode-only 部署，项目还支持一个一体化容器运行模式：
- 单个镜像同时打包 OpenCode、Redteam Agent 与渗透测试工具链
- 运行期密钥仍通过 `.env` / `--env-file` 注入，而不是写入镜像
- 容器内本地执行模式使用 `REDTEAM_RUNTIME_MODE=local`
- 传统宿主机安装模式仍保留，继续通过 Docker 子容器执行工具

如果你不想在宿主机直接安装 OpenCode 和工具链，可以使用该模式：
...
生成的 `run.sh` 会从镜像内置的干净模板初始化工作目录，并将运行状态持久化到安装目录下的 `workspace/`。
...
Phase 2: COLLECT ─ 导入 endpoint → SQLite 队列，启动 Katana 爬虫
    │
Phase 3: TEST ──── 消费队列 → vulnerability-analyst + source-analyzer
    │               exploit-developer 会并行处理 HIGH/MEDIUM finding
    │               （持续循环，并带进度显示）
...
在 `/exploit` 阶段，OpenCode 可能优先走可选的 Metasploit MCP 只读模块检索路径；只有当当前 finding 明显匹配某个模块族时，才会升级为有边界的模块执行。是否使用 Metasploit 仍由 `exploit-developer` 决策，不会对模糊 finding 把 Metasploit 当默认第一步。
