...
- **Orchestrator 图形界面** — 本地 Web UI，可查看项目、实时运行、产出物、时间线与终态运行元数据
...
- 最近的后端恢复逻辑也会在 supervisor 丢失或后端重启后自动续跑未完成任务，因此它适合长时间无人值守的运行场景。
...
| 拉取 Kali 包时 Docker 构建失败 | 重新执行构建。Dockerfile 现在会固定到官方 Kali 轮转入口并自动重试，但短暂的镜像/网络抖动仍可能需要再跑一次。 |
