...
**Primary optimization target:** The **RedTeam Agent project** (`agent/`, `orchestrator/`, root scripts). Fix `local-hermes-agent/` controller bugs if found.
...
- `/Users/cis/dev/projects/RedteamOpencode/local-hermes-agent/requirements/scan-optimizer-loop.md`
- `/Users/cis/dev/projects/RedteamOpencode/local-hermes-agent/state/optimizer-state.json` if present
- `/Users/cis/dev/projects/RedteamOpencode/local-hermes-agent/state/latest-context.md` if present
- `/Users/cis/dev/projects/RedteamOpencode/local-hermes-agent/recall-analysis/*.md` — all prior recall analysis reports (cumulative knowledge, build on prior work)
...
10. Read all prior recall analysis reports from `local-hermes-agent/recall-analysis/` to build on existing knowledge — do NOT repeat prior analysis.
...
12. Write a detailed recall analysis report to `local-hermes-agent/recall-analysis/YYYY-MM-DD-<cycle_id>.md`.
...
16. Scope: `agent/`, `orchestrator/`, `tests/`, root scripts, and `local-hermes-agent/` if bugs are found. Test scripts go in `tests/agent-contracts/`, NOT in `agent/scripts/`.
...
- No push.
- Primary target is the RedTeam Agent project. Fix `local-hermes-agent/` controller bugs if found.
- No public-repo documentation updates for local loop state.
- Keep local automation artifacts under `local-hermes-agent/` only.
