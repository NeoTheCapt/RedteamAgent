...
- Any changes to `local-hermes-agent/` audit scripts that would suppress or exclude findings
...
The following keys must be present in `local-hermes-agent/state/scheduler.env`:
...
HERMES_TIMEOUT_SECONDS=3000  # auditor cycles need >=2400s because Phase 1 UI walk + Phase 2 fixes + Phase 3 reverify routinely exceeds 20 min
```
...
`playwright`). `hermes_compat.sh` now auto-defaults the auditor skill
