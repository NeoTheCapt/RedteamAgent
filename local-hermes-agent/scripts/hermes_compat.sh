...
case "$SUBCOMMAND" in
  agent)
    session_id=""
    message=""
    timeout_seconds="${HERMES_TIMEOUT_SECONDS:-1800}"
    while [[ $# -gt 0 ]]; do
      case "$1" in
        --session-id)
          session_id="${2:-}"
    ...
    # Historical accident: controllers export HERMES_SKILL, but this shim
    ...
    # despite HERMES_SKILL=redteam-auditor-hermes). Fall back to
    # HERMES_SKILL so either name works.
    skill_name="${HERMES_SKILL:-${HERMES_SKILL:-scan-optimizer-hermes}}"
