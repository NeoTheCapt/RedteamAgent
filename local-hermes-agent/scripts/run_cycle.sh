...
if [[ -z "$HERMES_SKILL" ]]; then
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] HERMES_SKILL must be set (e.g. redteam-auditor-hermes or scan-optimizer-loop)" >&2
  exit 2
fi
...
hermes_log="$cycle_dir/hermes.log"
...
extract_auditor_sections() {
  ...
  python3 - \
      ...
      "$cycle_id" "${HERMES_TARGET_LOCAL:-}" "$review_src" \
...
if isinstance(bench_doc, dict):
    ...
    if not tgt:
        # Fall back to the first target if HERMES_TARGET_LOCAL wasn't set.
...
send_cycle_started_summary() {
  if [[ -z "$REPORT_CHANNEL" || -z "$REPORT_TO" || -z "$HERMES_BIN" ]]; then
    log "start delivery skipped (REPORT_CHANNEL / REPORT_TO / HERMES_BIN not fully set)"
    return 0
  fi
...
周期 ID: $cycle_id
启动时间: $start_at
调度间隔: ${LOCAL_HERMES_INTERVAL_SECONDS:-900}s
观察窗口: ${HERMES_OBSERVATION_SECONDS}s
目标:
- $HERMES_TARGET_OKX
- $HERMES_TARGET_LOCAL
...
EOF
  ...
  set +e
  "$HERMES_BIN" message send --channel "$REPORT_CHANNEL" --target "$REPORT_TO" --message "$(cat "$msg_file")" >> "$controller_log" 2>&1
...
send_cycle_summary() {
  if [[ -z "$REPORT_CHANNEL" || -z "$REPORT_TO" || -z "$HERMES_BIN" ]]; then
    log "summary delivery skipped (REPORT_CHANNEL / REPORT_TO / HERMES_BIN not fully set)"
    return 0
  fi
  ...
  if [[ "${HERMES_SKILL:-}" == "redteam-auditor-hermes" ]]; then
    local auditor_exit
...
INFRA
  ...
  if [[ -z "$body" ]] && [[ "${HERMES_SKILL:-}" == "redteam-auditor-hermes" ]]; then
  ...
  if [[ -z "$body" ]]; then
    body="$(extract_hermes_summary_raw || true)"
  fi
...
- reason: another cycle already holds the local-hermes-agent lock
- lock_dir: $LOCK_DIR
...
persist_cycle_state() {
  ...
  export HERMES_EXIT_CODE="$hermes_status"
...
sync_run_ids_from_json() {
  ...
  okx_run_id="$(jq -r "$_m"' [ .[] | select(tm("'"$HERMES_TARGET_OKX"'")) ] | last.id // empty' "$json_file" 2>/dev/null || true)"
  local_run_id="$(jq -r "$_m"' [ .[] | select(tm("'"$HERMES_TARGET_LOCAL"'")) ] | last.id // empty' "$json_file" 2>/dev/null || true)"
  okx_run_status="$(jq -r "$_m"' [ .[] | select(tm("'"$HERMES_TARGET_OKX"'")) ] | last.status // empty' "$json_file" 2>/dev/null || true)"
  local_run_status="$(jq -r "$_m"' [ .[] | select(tm("'"$HERMES_TARGET_LOCAL"'")) ] | last.status // empty' "$json_file" 2>/dev/null || true)"
}
...
log "hermes binary: ${HERMES_BIN:-missing}"
log "skill: $HERMES_SKILL"
send_cycle_started_summary
...
if [[ -z "$HERMES_BIN" ]]; then
  ...
  log "hermes binary not found in PATH"
...
elif [[ "${HERMES_SKILL:-}" == "redteam-auditor-hermes" \
  ...
  case "${HERMES_SKILL:-${HERMES_SKILL:-scan-optimizer-hermes}}" in
  ...
  if [[ $prep_status -ne 0 ]]; then
    ...
    prompt_file="$STATE_DIR/hermes-prompt.txt"
    ...
    while true; do
      ...
      if [[ "${SYNC_HERMES_SKILL:-1}" == "1" \
            && "${HERMES_SKILL:-}" != "redteam-auditor-hermes" ]]; then
        "$ROOT_DIR/scripts/sync_hermes_skill.sh" >> "$controller_log" 2>&1
      fi
      log "hermes attempt #$attempt_count starting"
      hermes_ran="true"
      ...
      if [[ $hermes_status -ne 0 ]]; then
        cycle_status="failed_hermes"
        log "hermes exited with code $hermes_status on attempt #$attempt_count"
      else
        cycle_status="success"
      fi

      if [[ -f "$hermes_log" ]]; then
        summary_excerpt="$(tail -n 120 "$hermes_log")"
      fi
      ...
      if grep -qF "$HERMES_HEALTHY_SKIP_MARKER" "$hermes_log" 2>/dev/null; then
        if [[ -n "$new_commit" ]]; then
          log "hermes reported healthy but also committed $new_commit; treating as success (commit takes precedence)"
          cycle_status="success"
        else
          cycle_status="skipped_healthy_runs"
          log "hermes completed all phases with no actionable issues; skipping this cycle"
        fi
        break
      fi
      ...
      if [[ -n "$new_commit" ]]; then
        if [[ $hermes_status -ne 0 ]]; then
          cycle_status="success_with_hermes_error"
          log "detected new local commit $new_commit on branch ${current_branch:-DETACHED} (hermes-runtime exited $hermes_status); cycle can finish"
        else
          cycle_status="success"
...
" 2>/dev/null || true
      ...
      log "hermes finished without a bug-fix commit and without an explicit healthy-skip marker"
      break
    done
  fi
fi
...
jq -n \
  ...
  --argjson hermes_exit_code "$hermes_status" \
...
- attempt_count: $attempt_count
- repo_root: $REPO_ROOT
- hermes_bin: ${HERMES_BIN:-missing}
- hermes_skill: $HERMES_SKILL
- hermes_timeout_seconds: ${HERMES_TIMEOUT_SECONDS}
- schedule_interval_seconds: ${LOCAL_HERMES_INTERVAL_SECONDS:-900}

## Fixed Targets
- okx: $HERMES_TARGET_OKX
- local: $HERMES_TARGET_LOCAL
...
- hermes_exit_code: $hermes_status
- hermes_ran: $hermes_ran
...
if [[ -n "$summary_excerpt" ]]; then
  ...
  echo '_no hermes output captured_' >> "$report_path"
fi
...
if [[ "${HERMES_SKILL:-}" == "redteam-auditor-hermes" ]] \
   && [[ -n "${new_commit:-}" ]] \
   && { [[ "$cycle_status" == "success" ]] || [[ "$cycle_status" == "success_with_hermes_error" ]]; } \
...
if [[ "${HERMES_SKILL:-}" == "redteam-auditor-hermes" ]]; then
  ...
  if [[ $artifact_exit -ne 0 || $regression_exit -ne 0 || $revert_exit -ne 0 || $steady_state_exit -ne 0 ]]; then
    if [[ "$cycle_status" == "success" || "$cycle_status" == "success_with_hermes_error" ]]; then
...
try:
    ...
    raise SystemExit
tgt = (d.get("targets") or {}).get("${HERMES_TARGET_LOCAL:-http://127.0.0.1:8000}")
if not tgt:
    raise SystemExit
...
PYEOF
  ...
  if [[ "$scored_this_cycle" == "yes" ]]; then
    ...
    python3 "$ROOT_DIR/scripts/recall_regression_report.py" \
        --target "${HERMES_TARGET_LOCAL:-http://127.0.0.1:8000}" \
...
case "$cycle_status" in
  success|success_with_hermes_error|skipped_healthy_runs)
    exit 0
    ;;
  *)
    exit 1
