...
case "$_local_status_now" in
    completed|complete|succeeded|success)
        ...
        if [[ -n "$completed_run_id" && "$completed_run_id" == "$last_scored_run_id" ]]; then
            ...
            HERMES_SKIP_JUICE_SHOP_SCORE=1 "$ROOT_DIR/scripts/build_context.sh" | tee "$LOGS_DIR/build-context.log"
        ...
        ;;
    *)
        OBSERVATION_WAIT="${HERMES_OBSERVATION_SECONDS:-300}"
...
cp "$PROMPTS_DIR/scan-optimizer-loop.txt" "$STATE_DIR/hermes-prompt.txt"
append_benchmark_context "$STATE_DIR/hermes-prompt.txt"

echo "[$(timestamp)] prepared OpenClaw cycle inputs:"
echo "- context: $STATE_DIR/latest-context.md"
echo "- prompt:  $STATE_DIR/hermes-prompt.txt"
