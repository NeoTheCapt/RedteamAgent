...
def test_auto_resume_bypasses_default_limit_for_continuous_report_hold(monkeypatch):
    client = TestClient(app)
    token = register_and_login(client, "alice-continuous-report-limit")
    project = create_project(client, token)
    run = create_run(client, token, project["id"], "https://www.example.com")

    run_root = Path(run["engagement_root"])
    workspace = run_root / "workspace"
    engagement_dir = workspace / "engagements" / "2026-03-31-000002-continuous-report-limit"
    engagement_dir.mkdir(parents=True, exist_ok=True)
    (workspace / "engagements" / ".active").write_text(
        "engagements/2026-03-31-000002-continuous-report-limit\n",
        encoding="utf-8",
    )
    (engagement_dir / "scope.json").write_text(
        json.dumps(
    ...
    metadata_path = run_root / "run.json"
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    metadata["auto_resume_count"] = 3
    metadata["auto_resume_progress"] = 2
    metadata_path.write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")
    ...
    project_obj = db.get_project_by_id(project["id"])
    user_obj = db.get_user_by_username("alice-continuous-report-limit")
    run_obj = db.get_run_by_id(run["id"])
    assert project_obj is not None
    assert user_obj is not None
    assert run_obj is not None
    ...
    assert resumed is True
    assert launched == {"container": 1, "supervisor": 1}
    latest = db.get_run_by_id(run["id"])
    assert latest is not None
    assert latest.status == "running"
    refreshed = json.loads(metadata_path.read_text(encoding="utf-8"))
    assert refreshed["auto_resume_count"] == 4
    events = db.list_events_for_run(run["id"])
    assert any("4/∞" in event.summary for event in events if event.event_type == "run.resumed")



def test_auto_resume_bypasses_limit_for_logged_observation_hold_with_stale_scope(monkeypatch):
    client = TestClient(app)
    token = register_and_login(client, "alice-continuous-stale-scope")
    project = create_project(client, token)
    run = create_run(client, token, project["id"], "https://www.example.com")

    run_root = Path(run["engagement_root"])
    workspace = run_root / "workspace"
    engagement_dir = workspace / "engagements" / "2026-04-26-000000-continuous-stale-scope"
    engagement_dir.mkdir(parents=True, exist_ok=True)
    (workspace / "engagements" / ".active").write_text(
        "engagements/2026-04-26-000000-continuous-stale-scope\n",
        encoding="utf-8",
    )
    (engagement_dir / "scope.json").write_text(
        json.dumps(
    ...
    (engagement_dir / "log.md").write_text(
        "## [13:59] Report complete — report-writer\n\n"
        "**Action**: report written; operator must enter continuous observation hold\n"
        "**Result**: /workspace/engagements/example/report.md\n\n"
        "## [14:02] Observation hold active — operator\n\n"
        "**Action**: entered continuous observation hold\n"
        "**Result**: runtime attached for https://www.example.com; heartbeat every 300s\n",
        encoding="utf-8",
    )
    with sqlite3.connect(engagement_dir / "cases.db") as connection:
        connection.execute("CREATE TABLE cases (id INTEGER PRIMARY KEY AUTOINCREMENT, status TEXT NOT NULL)")
        connection.executemany("INSERT INTO cases(status) VALUES (?)", [("pending",), ("done",)])
        connection.commit()
    ...
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    metadata["auto_resume_count"] = 3
    metadata["auto_resume_progress"] = 148
    metadata_path.write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")
    ...
    project_obj = db.get_project_by_id(project["id"])
    user_obj = db.get_user_by_username("alice-continuous-stale-scope")
    run_obj = db.get_run_by_id(run["id"])
    assert project_obj is not None
    assert user_obj is not None
    assert run_obj is not None
    ...
    resumed = _maybe_auto_resume_run(
        ...
        reason_text="Engagement status is in_progress.",
    )
...
def test_engagement_completion_state_promotes_report_phase_after_stale_stop_reason():
    client = TestClient(app)
    token = register_and_login(client, "alice-stale-stop")
    project = create_project(client, token)
    run = create_run(client, token, project["id"], "http://127.0.0.1:8000")

    run_root = Path(run["engagement_root"])
    workspace = run_root / "workspace"
    engagement_dir = workspace / "engagements" / "2026-03-28-000000-example-stale-stop"
    engagement_dir.mkdir(parents=True, exist_ok=True)
    (workspace / "engagements" / ".active").write_text(
        "engagements/2026-03-28-000000-example-stale-stop\n",
        encoding="utf-8",
    )
    (engagement_dir / "scope.json").write_text(
        json.dumps(
    ...
    (engagement_dir / "log.md").write_text(
        "# Engagement Log\n\n"
        "## [00:20] Run stop — operator\n\n"
        "**Action**: stop_reason=queue_incomplete\n"
        "**Result**: autonomous run paused with pending queue; resume required\n\n"
        "## [01:09] Report complete — report-writer\n\n"
        "**Action**: final report + finalize\n"
        "**Result**: report.md status=complete\n",
        encoding="utf-8",
    )
    (engagement_dir / "report.md").write_text(
        "# Penetration Test Report\n\n"
        ...
        "No confirmed findings were recorded in findings.md.\n\n"
        "## Attack Narrative\n"
        "The engagement resumed after a queue pause and completed reporting successfully.\n\n"
        "## Recommendations\n"
        "- Continue monitoring.\n\n"
        "## Appendix\n"
        "- cases.db rows: 2\n"
    ...
    assert "report" in normalized["phases_completed"]
