...
- if `pending > 0` and `processing = 0` immediately after a recent subagent handoff or an empty operator reply, treat that state as a dropped consume-test dispatch rather than a healthy pause; fetch the next real batch and launch the matching subagent in that SAME turn
