...
- Do **not** lump concrete observability / diagnostic endpoints into that low-signal bucket. When `/metrics`, `/health`, `/info`, `/status`, `/trace`, version/build dumps, or similar runtime introspection responses are in-scope and actually return data, preserve them as queue-worthy read cases or `file_handling`/data follow-up surfaces; they often leak secrets, config, dependency versions, and exploit pivots even if they are nominally read-only.
