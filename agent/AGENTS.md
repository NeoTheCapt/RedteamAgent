...
- A fetch+task pair counts as ONE advancing unit even though it is two tool calls. Therefore the "one advancing unit" rule never permits a standalone fetch-only assistant message; when `BATCH_COUNT` may be positive, the assistant message must already include the matching subagent `task(...)` call after the fetch command.
- This atomic fetch→task rule also applies to closure-gate work and any manual stage promotion. If you requeue/promote a CTF recall branch and then run `fetch_batch_to_file.sh` with `BATCH_COUNT>0`, the very next action in that same turn MUST be the matching `task(...)` dispatch for the printed `BATCH_AGENT`/`BATCH_FILE`. Never emit status-only text such as `[operator] Continuing closure batch.` after a non-empty closure fetch.
...
- Permission-stall hard guard: before submitting any bash/tool call in autonomous mode, scan the command for `/tmp`, `/tmp/*`, `mktemp`, `tee /tmp`, `> /tmp`, `cat /tmp`, bare absolute glob/search roots such as `/*` or `/**`, and unscoped recursive globs. If any are present, rewrite them to use `$DIR/tmp.operator` or an explicit workspace path in the same command; create `$DIR/tmp.operator` with `mkdir -p` before redirects, and never use `/tmp/*` as a capture/readback fallback after a closure-gate or log-entry command. A forbidden closure-gate command shape is `... dispatcher.sh "$DIR/cases.db" requeue ... >/tmp/dispatcher_requeue_235.out && ... cat /tmp/dispatcher_requeue_235.out`; rewrite that output capture to `$DIR/tmp.operator/dispatcher_requeue_<case>.out` before executing. Do not rely on OpenCode to ask/deny and recover later.
- Permission rejection recovery: if a tool output says `permission requested: external_directory` or mentions `/tmp/*` / `/*` in an auto-rejected command, treat that as a recoverable command-shape bug, not a reason to wait. The very next action in the SAME assistant turn must either rerun the identical bookkeeping/log command using `$DIR/tmp.operator` (or direct stdout) or write `Run stop — stop_reason=runtime_error` explaining the unrewritable path; never leave the run idle after a permission rejection.
...
In a single operator turn, you may issue MULTIPLE fetch+task pairs IF AND ONLY IF each pair is for a DIFFERENT (stage, agent) combination AND each pair is completed in strict sequence before starting the next pair: fetch #1 → immediately launch task #1 → then fetch #2 → immediately launch task #2. Never prefetch several batches first and then plan to dispatch later. Concrete:
- ✅ same turn, sequential ordering only: fetch-by-stage `ingested api 5 vulnerability-analyst` + task, then fetch-by-stage `vuln_confirmed api 3 exploit-developer` + task, then fetch-by-stage `fuzz_pending api 2 fuzzer` + task, then fetch-by-stage `ingested javascript 5 source-analyzer` + task
- ❌ same turn: fetch `ingested api` and fetch `ingested javascript` back-to-back before the vulnerability-analyst task — the second fetch orphans the first non-empty batch even if a task appears later
...
Each fetch must still be paired with the matching `task(...)` in the SAME turn before the next fetch and before the turn ends. The dispatcher's in-flight guard (refuses fetch when assigned_agent already has processing rows) prevents double-dispatching the same agent. Cross-stage parallelism is the design intent, but cross-stage concurrency is created by sequential fetch→task pairs, never by parallel or back-to-back fetch commands.
...
- a fetch assistant message is incomplete until it contains BOTH tool calls in order: first the `bash` call running `fetch_batch_to_file.sh`, then the matching `task(...)` subagent call using that exact `BATCH_AGENT` and `BATCH_FILE`. Never issue a one-tool message whose only tool call is the fetch command when `BATCH_COUNT` might be positive.
- never issue two `fetch_batch_to_file.sh` calls in the same assistant message, including via parallel/multi-tool execution. A non-empty fetch creates a dispatch contract immediately; a second API/source/other fetch before the first matching `task(...)` is an orphaned-batch bug even if both fetch tool calls finish in the same step.
- this is especially strict for API-family batches (`api`, `form`, `graphql`, `upload`, `websocket`): a non-empty fetch for `BATCH_AGENT=vulnerability-analyst` MUST be followed by the vulnerability-analyst task before any file read, queue scan, source batch, status text, another `fetch_batch_to_file.sh`, or final answer.
- this is especially strict for source-carrier types (`data`, `unknown`, `api-spec`, `javascript`, `stylesheet`, `page`): a non-empty fetch for `BATCH_AGENT=source-analyzer` MUST be followed by the source-analyzer task before any exploit follow-up, status text, queue scan, file read, API batch, or additional fetch. A fetched `data` carrier left in `processing` is an orphaned batch and will fail the run. A fetched `javascript` carrier left in `processing` without the matching source-analyzer handoff is also an orphaned batch and will fail the run.
...
The fresh 2026-05 recall regression set also requires exact second-pass closure branches for the peak-solved items that previously dropped together: `Bjoern's Favorite Pet` (Bjoern reset/security-answer candidate list from source/OSINT artifacts), `DOM XSS` and `Missing Encoding` (canonical browser hash-route payload plus Score Board visit, including the raw and percent-encoded backtick iframe forms), `Database Schema` and `User Credentials` (native login/search SQLi consumer replay with `sqlite_master` and credential-bearing fields, not just API roster evidence), `Deluxe Fraud` (paid membership/token workflow replay), `Forged Feedback` and `Zero Stars` (separate feedback mutations for forged identity/rating 0 and rating 5), `Login Admin` and `Password Strength` (canonical admin weak-password login/change branch), `Upload Size` and `XXE Data Access` (file-upload parser/size and XML data-access branches), `View Basket` (cross-user basket consumer), and `Meta Geo Stalking` (metadata-bearing image upload plus scoreboard refresh). If any of those names is false in the fresh challenge snapshot, the operator must requeue at least one named exact branch from this sentence before `report-writer`; a generic "no multi-step attack path" handoff is not terminal for these peak-retention items.
