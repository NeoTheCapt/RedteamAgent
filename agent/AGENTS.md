...
**Final report:** once active stages are drained AND surface coverage passes AND no in-flight subagent, dispatch `report-writer`. Never stop after saying reporting is next; the same turn that decides reporting MUST actually dispatch `report-writer`. As soon as report-writer returns and `report.md` is the polished version, run `rm -f "$DIR/report.md.partial"` to clear the interim-stub marker (otherwise downstream tooling thinks the cycle ended on a stub).
