...
if [[ "$count" -eq 1 ]]; then
  ...
  printf '%s\n' '{"request":{"method":"GET","endpoint":"https://www.example.com/en-sg","headers":{"Accept":"text/html"}},"response":{"status_code":200,"headers":{"Content-Type":"text/html"}}}' >> "$output"
  printf '%s\n' '{"request":{"method":"GET","endpoint":"https://www.example.com/priapi/v1/assistant/session/unread-message-count?t=1","headers":{"Accept":"application/json","X-Id-Group":"abc"}},"response":{"status_code":200,"headers":{"Content-Type":"application/json"}}}' >> "$output"
  printf '%s\n' '{"request":{"method":"POST","endpoint":"https://www.example.com/apmfe/api/206/batch/envelope/?project_key=test","body":"{}","headers":{"Content-Type":"application/json;charset=UTF-8"}},"response":{"status_code":200,"headers":{"Content-Type":"application/octet-stream"}}}' >> "$output"
fi
EOF
chmod +x "$FAKE_KATANA"
...
count_page="$(sqlite3 "$ENG_DIR/cases.db" "SELECT COUNT(*) FROM cases WHERE url='https://www.example.com/en-sg' AND source='katana';")"
...
count_api_get="$(sqlite3 "$ENG_DIR/cases.db" "SELECT COUNT(*) FROM cases WHERE url='https://www.example.com/priapi/v1/assistant/session/unread-message-count?t=1' AND source='katana-xhr';")"
...
count_api_post="$(sqlite3 "$ENG_DIR/cases.db" "SELECT COUNT(*) FROM cases WHERE url='https://www.example.com/apmfe/api/206/batch/envelope/?project_key=test' AND source='katana-xhr';")"
