...
REQUIRED_SNIPPETS = [
    ...
    'never issue two `fetch_batch_to_file.sh` calls in the same assistant message, including via parallel/multi-tool execution',
    'a second API/source/other fetch before the first matching `task(...)` is an orphaned-batch bug',
    'each pair is completed in strict sequence before starting the next pair: fetch #1 → immediately launch task #1 → then fetch #2 → immediately launch task #2',
    'fetch `ingested api` and fetch `ingested javascript` back-to-back before the vulnerability-analyst task',
    'Permission rejection recovery: if a tool output says `permission requested: external_directory`',
    'The very next action in the SAME assistant turn must either rerun the identical bookkeeping/log command using `$DIR/tmp.operator`',
    ...
    'a non-empty fetch for `BATCH_AGENT=vulnerability-analyst` MUST be followed by the vulnerability-analyst task before any file read, queue scan, source batch, status text, another `fetch_batch_to_file.sh`, or final answer',
