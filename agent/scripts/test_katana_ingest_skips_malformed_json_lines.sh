...
: > "$elog"
{
  ...
  printf '%s\n' '{not-json'
  printf '%s\n' '{"request":{"method":"GET","endpoint":"https://www.example.com/priapi/v1/dx/market/v2/token/pool/project/list","tag":"js","attribute":"regex","source":"https://www.example.com/cdn/assets/app.js"},"error":"hybrid: response is nil"}'
} > "$output"
EOF
chmod +x "$FAKE_KATANA"
...
assert rows[0]["request"]["endpoint"] == "https://www.example.com/en-sg", rows
assert rows[1]["request"]["endpoint"] == "https://www.example.com/priapi/v1/dx/market/v2/token/pool/project/list", rows
PY
