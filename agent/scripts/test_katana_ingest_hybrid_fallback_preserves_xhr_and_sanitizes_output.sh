...
if [[ "$count" -eq 1 ]]; then
  ...
  printf '%s' '{"request":{"method":"GET","endpoint":"https://www.okx.com/broken-tail","tag":"file","attribute":"robotstxt","source":"https://www.okx.com/robots.txt"},"response":{"status_code":404' >> "$output"
  sleep 2
else
...
pre_fallback_xhr="$(sqlite3 "$ENG_DIR/cases.db" "SELECT COUNT(*) FROM cases WHERE url LIKE 'https://www.okx.com/priapi/v1/dx/test/%' AND source = 'katana-xhr';")"
