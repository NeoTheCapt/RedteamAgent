...
1. **ASSESS STATE** — Read `scope.json` every loop, then inspect only the newest relevant slice of `log.md` / `findings.md` needed for the next decision. Run `./scripts/intel_changed_check.sh "$DIR"` once per loop; if it creates `.osint-respawn-required`, dispatch `osint-analyst` per Intel.md Rules before any other dispatch this turn (this is the only mechanical signal that intel.md grew — without it osint-analyst stays a ghost agent). Check recent `log.md` state before ANY action, but do NOT reload full long artifacts every turn unless you are preparing the final report or deduping a concrete finding.
...
If a branch remains blocked, requeue or log the exact blocker; do not let a generic all-cases-drained state hide the missing solved-state check.

**Final report:** once active stages are drained, CTF recall closure is satisfied (or concrete blockers are logged), surface coverage passes, and no in-flight subagent remains, dispatch `report-writer`. Never stop after saying reporting is next; the same turn that decides reporting MUST actually dispatch `report-writer`. As soon as report-writer returns and `report.md` is the polished version, run `rm -f "$DIR/report.md.partial"` to clear the interim-stub marker (otherwise downstream tooling thinks the cycle ended on a stub).
