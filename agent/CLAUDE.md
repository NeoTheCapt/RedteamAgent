...
- `Database Schema`: after SQL injection or admin data exposure, perform one schema-oriented extraction (`sqlite_master`, `information_schema`, or equivalent DB error path) and save the artifact. For Juice Shop, if `databaseSchemaChallenge` remains false after generic SQLi/admin evidence, requeue the native login/search injection with an explicit `sqlite_master` payload and a Score Board `/api/Challenges` check; do not substitute ORM stack traces or user roster dumps for the schema trigger.
