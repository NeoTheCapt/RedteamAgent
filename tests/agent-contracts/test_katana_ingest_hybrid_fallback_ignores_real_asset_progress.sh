...
if [[ "$count" -eq 1 ]]; then
  ...
  printf '%s\n' '{"request":{"method":"GET","endpoint":"https://www.okx.com/cdn/assets/app.js","tag":"script","attribute":"src","source":"https://www.okx.com/en-sg"},"response":{"status_code":200,"headers":{"Content-Type":"application/javascript"}}}' >> "$output"
  ...
  printf '%s\n' '{"request":{"method":"GET","endpoint":"https://www.okx.com/en-sg","tag":"navigation","attribute":"visit","source":"https://www.okx.com/en-sg"},"response":{"status_code":200,"headers":{"Content-Type":"text/html"},"xhr_requests":[{"method":"GET","endpoint":"https://www.okx.com/api/v5/market/tickers?instType=SPOT","source":"https://www.okx.com/en-sg","headers":{"Content-Type":"application/json"}}]}}' >> "$output"
fi
EOF
chmod +x "$FAKE_KATANA"
...
count_js="$(sqlite3 "$ENG_DIR/cases.db" "SELECT COUNT(*) FROM cases WHERE url = 'https://www.okx.com/cdn/assets/app.js' AND source = 'katana';")"
if [[ "$count_js" != "1" ]]; then
  echo "FAIL: expected the 200 JS asset row to be ingested once, got $count_js" >&2
  exit 1
fi

count_xhr="$(sqlite3 "$ENG_DIR/cases.db" "SELECT COUNT(*) FROM cases WHERE url = 'https://www.okx.com/api/v5/market/tickers?instType=SPOT' AND source = 'katana-xhr';")"
if [[ "$count_xhr" != "1" ]]; then
