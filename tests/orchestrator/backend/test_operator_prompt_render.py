...
def test_rendered_operator_prompt_blocks_surface_coverage_incomplete_stop(tmp_path: Path) -> None:
    ...
    assert "A `surface_coverage_incomplete` stop is forbidden" in rendered
    assert "need another bounded coverage pass" in rendered
    assert "must either ingest/fetch/dispatch that follow-up work" in rendered
    assert "Do not transition to `report`, emit `incomplete_stop`, or end the run" in rendered
