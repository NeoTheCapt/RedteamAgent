...
Look there FIRST. It will be something like `example.com` or `test.com --deep`.
...
1. Take the first word as the domain (e.g., `example.com`)
2. Strip any scheme (`http://`, `https://`), wildcard (`*.`), or trailing path (`/`)
3. Check for `--deep` flag

Examples:
- `example.com` → domain=`example.com`, deep=no
