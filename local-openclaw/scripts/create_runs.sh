...
latest_active_run_json() {
    local runs_json="$1"
    local target="$2"
    printf '%s' "$runs_json" | jq -c --arg target "$target" '
        [ .[] | select(.target == $target and (.status == "queued" or .status == "running")) ]
        | last // empty
    '
}
...
cleanup_target_runs() {
    ...
    while IFS=$'\t' read -r run_id run_status; do
        ...
        printf '%s' "$runs_json" | jq -r --arg target "$target" '
            .[]
...
ensure_single_active_run() {
    ...
    if [[ "$FORCE_REPLACE_ACTIVE_RUNS" == "1" ]]; then
        ...
        echo "force-replacing run for ${target}" >&2
        create_run "$target"
        return 0
    fi
