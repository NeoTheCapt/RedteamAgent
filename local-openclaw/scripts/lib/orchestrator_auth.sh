...
LOCAL_OPENCLAW_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
LOCAL_OPENCLAW_STATE_DIR="$LOCAL_OPENCLAW_ROOT/state"
LOCAL_OPENCLAW_REPO_ROOT="$(cd "$LOCAL_OPENCLAW_ROOT/.." && pwd)"
...
_refresh_orchestrator_token() {
  local cooldown_file="$LOCAL_OPENCLAW_STATE_DIR/.token-refresh-ts"
  local cooldown="${OPENCLAW_TOKEN_REFRESH_COOLDOWN_SECONDS:-30}"
  ...
  resolution_json="$(
    ...
    ORCH_AUTH_TTL_HOURS="${OPENCLAW_SESSION_TTL_HOURS:-24}" \
    python3 <<'PY'
import json
import os
import secrets
...
PY
  ...
  date +%s > "$LOCAL_OPENCLAW_STATE_DIR/.token-refresh-ts" 2>/dev/null || true
}
