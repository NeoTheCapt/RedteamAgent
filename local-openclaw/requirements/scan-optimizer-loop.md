...
The `local-openclaw/` directory is the controller itself — it is maintained separately and must NOT be modified by the optimization cycle.
...
6. Fix every confirmed issue in the **RedTeam Agent code** (`agent/`, `orchestrator/`, root scripts). Never modify `local-openclaw/`.
...
- Scope: `agent/`, `orchestrator/`, project root scripts. Do NOT review `local-openclaw/`.
